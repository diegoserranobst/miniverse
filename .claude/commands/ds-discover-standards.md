...
3. **First, output the draft as regular text** so the user can read it in full. Then use AskUserQuestion to ask for confirmation:

Output the draft as text:
```
...
Then ask with AskUserQuestion: "Create this file? (yes / edit: [your changes] / skip)"

4. Create or update the file in `openspec/standards/[folder]/`
...
1. Scan `openspec/standards/` for all `.md` files
...
3. Update `openspec/standards/index.yml`:
...
All standards: `openspec/standards/[folder]/[standard].md`
...
Output the draft as regular text so the user can see it, then ask with AskUserQuestion: "Create this file? (yes / edit / skip)"
