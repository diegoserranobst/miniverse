...
1. List all `.md` files in `openspec/standards/` and its subfolders
...
**Note:** `root` is a reserved keyword — it refers to `.md` files directly in `openspec/standards/` (not in a subfolder). Do not create an actual folder named "root".

### Step 2: Load Existing Index

Read `openspec/standards/index.yml` if it exists. Note which entries already have descriptions.
...
Generate `openspec/standards/index.yml` with this structure:
...
**Note:** `root` appears first and contains standards files that live directly in `openspec/standards/` (not in subfolders).
...
Updates `openspec/standards/index.yml`
